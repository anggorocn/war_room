...
$arrtabledata= array(
    ...
    , array("label"=>"Area", "field"=> "LIST_AREA_NAMA", "display"=>"",  "width"=>"20", "colspan"=>"", "rowspan"=>"")
    // , array("label"=>"Deskripsi", "field"=> "DESKRIPSI", "display"=>"",  "width"=>"20", "colspan"=>"", "rowspan"=>"")
    // , array("label"=>"Standar Referensi", "field"=> "STANDAR_NAMA", "display"=>"",  "width"=>"20", "colspan"=>"", "rowspan"=>"")
    ...
    , array("label"=>"fieldid", "field"=> "ITEM_ASSESSMENT_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);

$set= new Crud();
$statement=" AND KODE_MODUL ='0406'";
// $statement=" ";
...
while($set->nextRow())
{
    ...
    $arrdata["text"]= $set->getField("AREA_NAMA");
...
<script type="text/javascript">
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/master_item_assessment_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/master_item_assessment_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/item_assessment_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
    ...
    $('#btnImport').on('click', function () {
        openAdd("app/index/master_item_assessment_import");
    });
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/item_assessment_json/json?reqPencarian="+reqPencarian+"&reqArea="+reqArea+"&reqStatus="+reqStatus;
    ...
    $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/item_assessment_json/delete/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/item_assessment_json/json";
