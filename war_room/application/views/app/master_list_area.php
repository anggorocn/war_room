...
$arrtabledata= array(
    ...
    , array("label"=>"fieldid", "field"=> "LIST_AREA_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);

$set= new Crud();
$statement=" AND KODE_MODUL ='0407'";
$kode= $appuserkodehak;
...
<script type="text/javascript">
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/master_list_area_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/master_list_area_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });

    $('#btnImport').on('click', function () {
        openAdd("app/index/master_list_area_import");
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/list_area_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
    ...
    $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/list_area_json/delete/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/list_area_json/json?reqPencarian="+reqPencarian+"&reqKode="+reqKode+"&reqStatus="+reqStatus;
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/list_area_json/json";
