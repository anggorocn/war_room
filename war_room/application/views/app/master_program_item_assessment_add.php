...
else
{
	...
	$statement = " AND PROGRAM_ITEM_ASSESSMENT_ID = '".$reqId."' ";
    $set->selectByParams(array(), -1, -1, $statement);
    // echo $set->query;exit;
    $set->firstRow();
    $reqId= $set->getField("PROGRAM_ITEM_ASSESSMENT_ID");
    $reqNama= $set->getField("NAMA");
    $reqKode= $set->getField("KODE");
    $reqRating= $set->getField("RATING");
                ...
                <form id="ff" class="easyui-form form-horizontal" method="post" novalidate enctype="multipart/form-data">
                   ...
                   <div class="form-group">  
                        ...
                        <div class='col-md-8'>
                            <div class='form-group'>
                                <div class='col-md-11'>
                                   <input autocomplete="off" class="easyui-validatebox textbox form-control" type="text" min="0" max="1" maxlength="1"  name="reqRating"  id="reqRating" value="<?=$reqRating?>" <?=$disabled?> style="width:100%" />
                               </div>
                           </div>
                       </div>
                   </div>
...
document.getElementById('reqRating').addEventListener('input', function (e) {
...
function submitForm(){
    ...
    $('#ff').form('submit',{
        url:'json-app/program_item_assessment_json/add',
        onSubmit:function(){
