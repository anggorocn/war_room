...
$arrtabledata= array(
    ...
    , array("label"=>"fieldid", "field"=> "PERATURAN_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);

$set= new Crud();
$statement=" AND KODE_MODUL ='0305'";
// $statement=" ";
...
<script type="text/javascript">
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/master_peraturan_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/master_peraturan_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        if(valinfostatus=='' || valinfostatus==null )
        {
            ...
            // var pesan='Non Aktifkan data terpilih?';
             var pesan='Data tidak bisa di non Aktifkan, salah satu data harus ada yang aktif';
             $.messager.alert('Info',pesan, 'warning');
             return false;
        }
        else
        {
        ...
        $.messager.confirm('Status Aktif',pesan,function(r){
            if (r){
                $.getJSON("json-app/peraturan_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
    ...
    $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/peraturan_json/delete/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
    ...
    $('#btnImport').on('click', function () {
        openAdd("app/index/master_peraturan_import");
    });
	...
	$('#btnCari').on('click', function () {
        ...
        jsonurl= "json-app/peraturan_json/json?reqPencarian="+reqPencarian+"&reqStatus="+reqStatus;
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/peraturan_json/json";
