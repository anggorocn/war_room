...
$arrtabledata= array(
    ...
    , array("label"=>"fieldid", "field"=> "STATUS_REKOMENDASI_ID", "display"=>"1",  "width"=>"", "colspan"=>"", "rowspan"=>"")
);

$set= new Crud();
$statement=" AND KODE_MODUL ='0505'";
//$statement=" ";
...
<script type="text/javascript">
	...
	$("#btnAdd, #btnEdit").on("click", function () {
        ...
        varurl= "app/index/master_status_rekomendasi_add?reqId="+valinfoid;
    ...
    $("#btnLihat").on("click", function () {
        ...
        varurl= "app/index/master_status_rekomendasi_add?reqId="+valinfoid+"&reqLihat=1";
        document.location.href = varurl;
    });
    ...
    $('#btnDelete').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/status_rekomendasi_json/update_status/?reqId="+valinfoid+"&reqStatus="+reqStatus,
                    function(data){
    ...
    $('#btnDeleteNew').on('click', function () {
        ...
        $.messager.confirm('Konfirmasi',pesan,function(r){
            if (r){
                $.getJSON("json-app/status_rekomendasi_json/delete/?reqId="+valinfoid,
                    function(data){
                        $.messager.alert('Info', data.PESAN, 'info');
                        valinfoid= "";
                        setCariInfo();
    ...
    $('#btnImport').on('click', function () {
        openAdd("app/index/master_status_rekomendasi_import");
    });

	$('#btnCari').on('click', function () {
		reqPencarian= $('#example_filter input').val();
        reqStatus=$('#reqStatus').val();
        jsonurl= "json-app/status_rekomendasi_json/json?reqPencarian="+reqPencarian+"&reqStatus="+reqStatus;
	...
	jQuery(document).ready(function() {
		var jsonurl= "json-app/status_rekomendasi_json/json";
