...
class outlining_assessment_approval_json extends CI_Controller
{
	...
	function json()
	{
		...
		// $statement=" AND (A.V_STATUS = 0 OR A.V_STATUS = 1 OR A.V_STATUS = 10 OR A.V_STATUS = 30)
		//  AND NOT  EXISTS
	 //        (
	 //          SELECT ROLE_ID FROM APPRDETAIL X 
	 //          WHERE X.APPR_ID= E.APPR_ID 
	 //          AND  X.ROLE_ID = '".$appuserroleid."'
	 ...
	 //        )";
		...
		$set->selectByParamsApproval(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);
			...
			foreach($columnsDefault as $valkey => $valitem) 
			{
				...
				if(isset($detaildok))
				{
					...
					elseif($dokinfostatus == 10 || $dokinfostatus==20)
					{
						...
						{
							// $statusapproval .= '<br> - '.$rows['NAMA'].' Pada Tanggal : '.$rows['APRD_TGL'];
