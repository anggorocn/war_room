...
class program_item_assessment_json extends CI_Controller
{
	...
	function add()
	{
		...
		$set = new ProgramItemAssessment();
		$set->setField("PROGRAM_ITEM_ASSESSMENT_ID", $reqId);
		$set->setField("NAMA", $reqNama);
		$set->setField("KODE", $reqKode);
		$set->setField("RATING", ValToNullDB($reqRating));
		...
		if ($reqMode == "insert")
		{
			...
			$check = new ProgramItemAssessment();
