...
class penanganan_risiko_json extends CI_Controller
{
	...
	function json()
	{
		...
		$sOrder = " ORDER BY A.PENANGANAN_RISIKO_ID ASC";
	...
	function add()
	{
		...
		$set = new PenangananRisiko();
		$set->setField("PENANGANAN_RISIKO_ID", $reqId);
		$set->setField("KODE_WARNA", $reqWarna);
		$set->setField("RISIKO_ID", $reqRisikoId);
		$set->setField("STATUS", ValToNullDB($reqStatus));

		if ( empty($reqRisikoId))
		{
			echo "xxx***Risiko harus diisi";exit;
		}
