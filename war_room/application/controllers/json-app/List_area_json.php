...
class list_area_json extends CI_Controller
{
	...
	function jsonduplikat()
	{
		...
		$sOrder = " ORDER BY A.NAMA ASC";
		$set->selectduplikat(array(), $dsplyRange, $dsplyStart, $statement.$searchJson, $sOrder);
	...
	function add()
	{
		...
		$set = new ListArea();
		$set->setField("LIST_AREA_ID", $reqId);
		...
		if ($reqMode == "insert")
		{
			...
			if(!empty($checkKode))
			{
				echo "xxx***Kode List Area ".$checkKode." sudah ada";exit;	
			}
	...
	function getData()
	{
		...
		$statement = " AND LIST_AREA_ID = '".$reqAreaId."' ";
