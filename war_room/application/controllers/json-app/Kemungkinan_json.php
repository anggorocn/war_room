...
class kemungkinan_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqNmin= $this->input->post("reqNmin");
		$reqNmax= $this->input->post("reqNmax");
		// print_r($reqKemungkinanId);exit;

		$set = new Kemungkinan();
		$set->setField("KEMUNGKINAN_ID", $reqId);
		$set->setField("NAMA", $reqNama);
		$set->setField("BOBOT", ValToNullDB($reqBobot));
		$set->setField("KODE", $reqKode);
		$set->setField("PERATURAN_ID", $reqPeraturanId);

		$set->setField("N_MIN", $reqNmin);
		$set->setField("N_MAX", $reqNmax);
