...
class Crud_json extends CI_Controller
{
	...
	function addDurasi()
	{
		...
		$set = new Crud();
		$set->setField("durasi_autoplay", $reqDurasi);
		if($set->updateDurasi()){
	...
	function addPilihanKinerja()
	{
		$this->load->model("base-admin/Crud");

		$id_kinerja= $this->input->post("id_kinerja");
		$urutan_kinerja= $this->input->post("urutan_kinerja");
		// print_r($id_kinerja);exit;

		$set = new Crud();
		$set->deletePilihanKinerja();

		for($i=0;$i<count($id_kinerja);$i++){
			$set->setField("Kinerja_id", ValToNullDB($id_kinerja[$i]));
			$set->setField("Urutan", ValToNullDB($urutan_kinerja[$i]));
			$set->insertPilihanKinerja();
		}
	...
	function deletePilihanKinerja()
	{
		$this->load->model("base-admin/Crud");
		$set = new Crud();
		$reqId =  $this->input->get('reqId');

		if($set->deletePilihanKinerja('and Kinerja_id='.$reqId))
		{
