...
class area_assessment_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqAreaNama= $this->input->post("reqAreaNama");

		$reqNama= $this->input->post("reqNama");
		$reqGrouping= $this->input->post("reqGrouping");

		$reqAssetId= $this->input->post("reqAssetId");
		$reqAssetNama= $this->input->post("reqAssetNama");
		
		$reqSourceId= $this->input->post("reqSourceId");
		$reqSourceNama= $this->input->post("reqSourceNama");
		...
		$set = new AreaAssessment();
		$set->setField("AREA_ASSESSMENT_ID", $reqId);
		$set->setField("NAMA", $reqNama);
		$set->setField("GROUPING", $reqGrouping);

		// $set->setField("AREA_ID", ValToNullDB($reqaaAreaId));

		$set->setField("ASSET_ID", ValToNullDB($reqAssetId));
		$set->setField("LOCATION_ID", ValToNullDB($reqSourceId));
		...
		if(!empty($reqAreaId))
		{
			$setinsert = new AreaAssessment();
			$setinsert->setField("AREA_ASSESSMENT_ID", $reqId);
			$setinsert->deletearea();
			foreach ($reqAreaId as $key => $value) {
				$setinsert->setField("AREA_ID", $value);
				$setinsert->setField("AREA_ASSESSMENT_ID", $reqId);

				if($setinsert->insertarea())
				{
					$reqSimpan= 1;
				}
