...
class location_json extends CI_Controller
{
	...
	function json()
	{
		...
		$sOrder = " ORDER BY A.KODE_LOCATION ASC ";
	...
	function add()
	{
		...
		$set = new Location();
		$set->setField("LOCATION_ID", $reqId);
		$set->setField("EAM_ID", ValToNullDB($reqEamId));
		$set->setField("KODE_LOCATION", $reqKode);
		$set->setField("DESKRIPSI_LOCATION", $reqDeskripsi);
		...
		if ($reqMode == "insert")
		{
			...
			$statement=" AND A.KODE_LOCATION =  '".$reqKode."' ";
			$check = new Location();
			$check->selectByParams(array(), -1, -1, $statement);
			// echo $check->query;exit;
			$check->firstRow();
			$checkKode= $check->getField("KODE_LOCATION");

			if(!empty($checkKode))
			{
				echo "xxx***Kode Location ".$checkKode." sudah ada";exit;	
			}
