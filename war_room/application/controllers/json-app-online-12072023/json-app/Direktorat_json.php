...
class direktorat_json extends CI_Controller
{
	...
	function add()
	{
		...
		// print_r($reqWilayahId);exit;

		$set = new Direktorat();
		$set->setField("DIREKTORAT_ID", $reqId);
		...
		{
			$setdelete = new Direktorat();
			$reqSimpan="";
			$setdelete->setField("DIREKTORAT_ID", $reqId);
			$setdelete->deletewilayah();
			foreach ($reqWilayahId as $key => $value) 
			{
				$setpengukuran = new Direktorat();
				$setpengukuran->setField("WILAYAH_ID", $value);
				$setpengukuran->setField("DIREKTORAT_ID", $reqId);

				if($setpengukuran->insertwilayah())
				{
					$reqSimpan= 1;
				}
			}
