...
class Login extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->darkmode= $this->session->userdata("darkmode");
		$this->appusername= $this->session->userdata("appusername");
		$this->apppassword= $this->session->userdata("apppassword");
		$this->appautoconnect= $this->session->userdata("appautoconnect");
	}
	...
	function ganti_darkmode(){
		$checkbox = $this->input->get('checkbox');
		// $reqBahasa = $reqBahasa?$reqBahasa:'id';
		
		$this->session->set_userdata('darkmode', $checkbox);
	}
			...
			if($valid == 1)
			{
				...
				else
				{
					...
					if($insert->insert())
					{
						...
						if($reqSimpan==1)
						{
							$insertpage = new Pengguna();
							
							$insertpage->setField("PENGGUNA_ID", $insert->id);
							$insertpage->setField("PENGGUNA_PAGE_HAK_ID", 2);
							$reqSimpan="";
							if($insertpage->insertPenggunaPageHakAkses())
							{
								$reqSimpan=1;
							}	
						}
	...
	function resetsess()
	{
		// if($this->appautoconnect==1)
		// {
			$respon = $this->kauth->cekuserapp($this->appusername,$this->apppassword);
			if($respon==1)
			{
				echo " Session telah diperbarui";
			}
			else
			{
				echo " Session gagal diperbarui";
			}
