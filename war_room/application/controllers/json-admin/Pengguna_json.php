...
class Pengguna_json extends CI_Controller
{
	...
	function add()
	{
		...
		$reqHakPage= $this->input->post("reqHakPage");

		$reqAuto= $this->input->post("reqAuto");
		...
		$set->setField("AUTO_CONNECT", valToNullDB($reqAuto));
		...
		if($reqSimpan == 1 )
		{
			...
			if(!empty($reqHakPage))
			{
				$setinsert = new Pengguna();
				$setinsert->setField("PENGGUNA_ID", $reqId);
				$setinsert->deletePenggunaPageHakAkses();
				foreach ($reqHakPage as $key => $value) {
					
					$setinsert->setField("PENGGUNA_PAGE_HAK_ID", $value);
					
					if($setinsert->insertPenggunaPageHakAkses())
					{
						$reqSimpan= 1;
					}
